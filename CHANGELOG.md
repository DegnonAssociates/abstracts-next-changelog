...
- Documentation: Refresh local setup and verification instructions and simplify TypeScript guidance while preserving existing compiler and lint settings.

## 2026-09-16

- Update: Clarify that the question inclusion checkbox applies to the submission type’s overall character/word limit.
- Feature: Add optional word or character limits to individual paragraph fields, with live response counts and enforcement on save and final submission, independently of the overall submission limit. Uses existing field settings without a database migration.
...
- Update: Separated Main author and Presenting author assignments so each badge can appear independently or together without presenter changes altering submission ownership.
...
- Update: Included ordered author names, degrees, and organizations in parent and child submission confirmation summaries.
- Update: Allowed each author to be marked independently as presenting, supporting zero, one, or multiple presenting authors without reassignment guardrails.
- Fix: Kept conditionally displayed submission questions visible after saving when their controlling radio-button answer remains selected.
